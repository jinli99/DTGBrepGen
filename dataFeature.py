...
def compute_laplacian(adjacency, normalize: bool):
    ...
    normalize: can be None, 'sym' or 'rw' for the combinatorial, symmetric normalized or random walk Laplacian
    Return:
...
def get_eigenvectors_features(vectors, node_mask, n_connected, k=2):
    ...
    returns:
        ...
        k_lowest_eig_vec : k first eigenvectors for the largest connected component   -- (bs, n, k)
    """
...
class KNodeCycles:
    ...
    def k6_cycle(self):
        ...
        term_3_t = torch.sum(self.adj_matrix * self.k2_matrix.pow(2), dim=[-2, -1])
        ...
        term_8_t = torch.sum(self.k3_matrix, dim=[-2, -1])
        term_9_t = batch_diagonal(self.k2_matrix).pow(2).sum(-1)
        term_10_t = batch_trace(self.k2_matrix)
    ...
    def k_cycles(self, adj_matrix, verbose=False):
        ...
        k_cycles_x = torch.cat([k3x, k4x, k5x], dim=-1)
        k_cycles_y = torch.cat([k3y, k4y, k5y, k6y], dim=-1)
        return k_cycles_x, k_cycles_y


class NodeCycleFeatures:
    def __init__(self):
        self.k_cycles = KNodeCycles()

    def __call__(self, e, node_mask):
        adj_matrix = e[..., 1:].sum(dim=-1).float()

        x_cycles, y_cycles = self.k_cycles.k_cycles(adj_matrix=adj_matrix)   # (bs, n_cycles)
...
class EigenFeatures:
    ...
    def __call__(self, e, node_mask):
        ...
        if self.mode == 'eigenvalues':
            eig_vals = torch.linalg.eigvalsh(L)        # bs, n
            eig_vals = eig_vals.type_as(A) / torch.sum(mask, dim=1, keepdim=True)

            n_connected_comp, batch_eigenvalues = get_eigenvalues_features(eigenvalues=eig_vals)
            return n_connected_comp.type_as(A), batch_eigenvalues.type_as(A)

        elif self.mode == 'all':
            eig_vals, eig_vectors = torch.linalg.eigh(L)
            eig_vals = eig_vals.type_as(A) / torch.sum(mask, dim=1, keepdim=True)
            eig_vectors = eig_vectors * mask.unsqueeze(2) * mask.unsqueeze(1)
            # Retrieve eigenvalues features
            n_connected_comp, batch_eigenvalues = get_eigenvalues_features(eigenvalues=eig_vals)

            # Retrieve eigenvectors features
            non_lcc_indicator, k_lowest_eigenvector = get_eigenvectors_features(
                vectors=eig_vectors, node_mask=mask, n_connected=n_connected_comp)
            return n_connected_comp, batch_eigenvalues, non_lcc_indicator, k_lowest_eigenvector
        else:
...
class GraphFeatures:
    def __init__(self, extra_features_type, max_n_nodes):
        ...
        self.n_cycles = NodeCycleFeatures()
        self.features_type = extra_features_type
        if extra_features_type in ['eigenvalues', 'all']:
            self.eigen_features = EigenFeatures(mode=extra_features_type)
    ...
    def __call__(self, e, node_mask):
        ...
        if self.features_type == 'cycles':
            ...
            eigen_features = self.eigen_features(e, node_mask)
            extra_edge_attr = torch.zeros((*e.shape[:-1], 0)).type_as(e)
            n_components, batched_eigenvalues = eigen_features   # (bs, 1), (bs, 10)
            return x_cycles, extra_edge_attr, torch.hstack((n, y_cycles, n_components, batched_eigenvalues))
        elif self.features_type == 'all':
            eigen_features = self.eigen_features(e, node_mask)
            extra_edge_attr = torch.zeros((*e.shape[:-1], 0)).type_as(e)
            n_components, batched_eigenvalues, non_lcc_indicator, k_lowest_eig_vec = eigen_features

            return (torch.cat((x_cycles, non_lcc_indicator, k_lowest_eig_vec), dim=-1), extra_edge_attr,
