...
def get_topology(files, device, face_vae_encoder):
    ...
    face_geom = face_latent.flatten(-2, -1)
    ...
    return {"face_mask": face_mask, "edgeVert_adj": edgeVert_adj, "edgeFace_adj": edgeFace_adj,
            "face_geom": face_geom, "face_bbox": face_bbox, "vert_geom": vert_geom, "vert_mask": vert_mask}
...
def main():
    ...
    for i in tqdm(range(0, len(batch_file), b_each)):
        ...
        face_geom, face_bbox, face_mask, edgeVert_adj, edgeFace_adj, vert_geom, vert_mask = (datas["face_geom"],
                                                                                             datas["face_bbox"],
                                                                                             datas["face_mask"],
                                                                                             datas["edgeVert_adj"],
                                                                                             datas["edgeFace_adj"],
