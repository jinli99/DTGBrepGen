...
class FaceEdgeTrainer:
    def __init__(self, args, train_dataset, val_dataset):
        ...
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.save_dir = args.save_dir
        model = FaceEdgeModel(max_face=args.max_face)
        model = nn.DataParallel(model)  # distributed training
        self.model = model.to(self.device).train()
        self.train_dataloader = torch.utils.data.DataLoader(train_dataset,
                                                            shuffle=True,
    ...
    def train_one_epoch(self):
        ...
        for data in self.train_dataloader:
            with torch.cuda.amp.autocast():
                ...
                nf = torch.nonzero(mask.int().sum(0), as_tuple=True)[0][-1].cpu().item() + 1
                fe_topo = fe_topo[:, :nf, :nf]
                mask = mask[:, :nf]

                logits = self.model(fe_topo, mask)
